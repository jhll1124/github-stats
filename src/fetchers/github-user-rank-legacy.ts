import { UserStats } from "./github-user-rank.ts";
...
export function calculateRankLegacy({
  repos,
  commits,
  contributions,
  followers,
  ...
  const ALL_OFFSETS = CONTRIBS_OFFSET +
    ISSUES_OFFSET +
    STARS_OFFSET +
    PRS_OFFSET +
    FOLLOWERS_OFFSET +
  ...
  const TOTAL_VALUES = RANK_S_VALUE + RANK_A2_VALUE + RANK_A3_VALUE +
    RANK_B_VALUE;

  const score = (commits * COMMITS_OFFSET +
